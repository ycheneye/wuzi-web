...
<jsp:include page="public/top.jsp"></jsp:include>
	...
	<table id="grid0" title="借用查询" class="easyui-datagrid" fitColumns="true" pageSize="20"
	    ...
	    <thead>
	    	<tr>
	    		...
	    		<th field="op" width="150" align="center" formatter = "recessionOp">管理员操作</th>
	...
	<script>
		...
		function recession(id,goodsid,count){
			if(grid0.getSelections()[0].state == 5){
				$.messager.alert('系统提示','<font color=red>已处于待审核状态，请勿重复提交！</font>');
				return;
				
			}
			
			$.post("${basePath}/admin/dept/recession.do",{id:id,goodsid:goodsid,count:count },function(data){
				if(data.errCode < 0){
					$.messager.alert('系统提示','<font color=red>'+data.errMsg+'</font>');
					return;
				}
				
				$.messager.alert('系统提示','<font color=green>提交成功！</font>');
				grid0.reload();
